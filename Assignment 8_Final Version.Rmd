...
The second libary that will be loaded is the **gapminder**, which is a package that contains the information that we want to work on. 

```{r}
library(gapminder)
```

In order to manipulte the datasets the **dplyr** package needs to be intalled. 
...
The Visual of graphs are important, so we can include on our comand the instructions to have a title (main=) and proper labes for each axis (xlab=;ylab=).
...
To creat a plot in order to correlate the life expectancy and GDP per capita we will use the following comands.
...
Let's suppose that we want to work with the data from South Africa and Ireland. So, I'll creat a SFXI (South Africa vs Ireland) to perform the ttest of life expectancy between those two countries. 
